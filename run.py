...
def rand_loc(loc: list, d=0.000025, n=5):
    result = [i.copy() for i in loc]
    ...
    for i in range(n):
        ...
        for j in range(start, end):
            distance = math.sqrt((result[j]["lat"] - center["lat"])**2 +
                                 (result[j]["lng"] - center["lng"])**2)

            if distance == 0:
                continue

            factor = offset * smooth(start, end, j) / distance
            result[j]["lat"] += (result[j]["lat"] - center["lat"]) * factor
            result[j]["lng"] += (result[j]["lng"] - center["lng"]) * factor
...
def fix_lock_t(loc: list, v, dt):
    ...
    while t < T[0]:
        ...
        fixed_loc.append({"lat": xa, "lng": xb})
        j += 1
        t += dt
    ...
    for i in range(1, len(loc)):
        ...
        while t < T[i]:
            xa = a["lat"] + j * (b["lat"] - a["lat"]) / max(
                1, int((T[i] - T[i - 1]) / dt))
            xb = a["lng"] + j * (b["lng"] - a["lng"]) / max(
                1, int((T[i] - T[i - 1]) / dt))
            fixed_loc.append({"lat": xa, "lng": xb})
            j += 1
            t += dt
...
def run(dvt, version, loc: list, v, d=15, dt=0.2):
    random.seed(time.time())

    while True:
        v_rand = 1000 / (1000 / v - (2 * random.random() - 1) * d)
        fixed_loc = fix_lock_t(loc, v_rand, dt)
        n_list = [5, 6, 7, 8, 9]
        n = n_list[random.randint(0, len(n_list) - 1)]
        fixed_loc = rand_loc(fixed_loc, n=n)
        clock = time.time()

        for i in fixed_loc:
            if int(version.split(".")[0]) >= 17:
                location.set_location_17(dvt, **bd09_to_wgs84(i))
            else:
                location.set_location(dvt, **bd09_to_wgs84(i))
